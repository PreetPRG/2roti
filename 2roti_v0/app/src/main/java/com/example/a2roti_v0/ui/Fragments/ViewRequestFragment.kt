 ...
 */
class ViewRequestFragment : Fragment(),DonationsAdapter.OnDonationclickListener {
    ...
    private fun setUpRecyclerView(flag:Int) {
        ...
        db.collection("Users").document(FirebaseAuth.getInstance().currentUser!!.uid).get()
            .addOnSuccessListener {
                ...
                requests_recyclerview.layoutManager = LinearLayoutManager(activity)
                requests_recyclerview.adapter=adapterr
            }
    ...
    override fun OnDonationClick(documentSnapshot: DocumentSnapshot?, position: Int) {
        ...
        startActivity(Intent(activity, viewDetailsActivity::class.java).putExtra("id",documentSnapshot.id).putExtra("flag",false))
